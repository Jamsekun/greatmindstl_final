...
@section('page-head')
<div id="page-title">
    <h1 class="page-header text-overflow">Lottery Expenses</h1>
</div>
...
<div class="row pad-btm">
    <div class="col-sm-6 toolbar-left">
        @can('expenses.create')
            <a href="{{ route('admin.expenses.create') }}" id="demo-btn-addrow" class="btn btn-purple">Add New</a>
        @endcan
    </div>
</div>
...
$(function (namespace, $) {
    ...
    p.EditDetails = function (o) {
        if(!p.IsEditable()) {
            toastr.error('You don\'t have permission to edit expenses data.', 'Error');
    ...
    p._DataTable = function () {
    	dt = $('#datatable-expenses').DataTable({
    		responsive: true,
        	language: {
            	paginate: {
        		...
        		{ 
                    ...
                    searchable: false,
        		  	render: function (data, type) {
        		  		return '\
	                        <a href="javascript:;" onclick="js.EditDetails(this)" class="btn btn-md btn-clean btn-icon-md" data-item-info="edit-details" data-id="' + data.id + '" data-url="' + '{{ route('admin.expenses.edit', ':id') }}' + '" title="Edit details">\
